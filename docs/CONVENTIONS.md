...
- Timestamps: `datetime` + `@field_serializer` → `TimestampStr` via `fmt_ts()`; `Z`, six-digit fraction; never hand-built. Three sweeps in `tests/api/public/test_timestamp_serialization.py` (#440) cover pre-serialization, serializer coverage, and the published OpenAPI shape. Full rules → `docs/CONVENTIONS.md` §"Public API"
