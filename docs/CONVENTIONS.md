...
- Timestamps: `datetime` fields + `@field_serializer` returning `TimestampStr`/`TimestampStr | None` (matching the field) via `fmt_ts()`; ISO 8601 UTC, fixed six-digit fraction, `Z` suffix; never hand-build a timestamp `str`. Four sweeps in `tests/api/public/test_timestamp_serialization.py` enforce it (#440), over `src/api/public/*.py`: **(1)** nothing outside `schemas.py` may **call `fmt_ts`** (bare or module-qualified) and nothing anywhere may call `.isoformat()` outside `fmt_ts` itself (`<dt>.date().isoformat()` exempt) — formatting in a handler is the breach whatever the field is named, which is how `PartialDate.at` hid; **(1b)** no `str`-typed field named like a timestamp (`at`, `…_at`, `…_time`, `timestamp`; aliases such as `TimestampStr` resolved) on a response model — a name-based backstop, not the guard; **(2)** an `fmt_ts` serializer on every `datetime` reachable from a `response_model=`, request models exempt **by reachability, never by name**; **(3)** the published OpenAPI shape matching the field, since the serializer's *return* annotation is what ships — hence `TimestampStr` (it carries `format: date-time`, which a bare `-> str` erases) and a split serializer wherever one spans required *and* optional fields. `fmt_ts` normalizes nothing else: a naive or `+05:00` value passes through unchanged. The module docstring carries the reasoning and the ratchet caveats; the consumer-facing promise is `docs/PUBLIC_API.md` §"Timestamps" — change the two together
...
- Observation atomicity (#456): identity resolution and every payload write share **one transaction** on the four identity-observation endpoints (people, orgs, jurisdictions, assignments), so a `rejected` disposition means **nothing persisted — the entity included**. This needs saying because the identity half *creates*: `resolve_entity` mints the entity on an unseen identifier before any payload is written, so resolution running outside the transaction left a bare entity committed behind a rejection whose response withholds `entity_id` — unreachable by the caller. The endpoint's kind guard had the same hole (an `org_ubi` identifier on `/people/observations` minted an Organization, *then* rejected). Both guards therefore **raise** `ObservationRejected` inside the transaction instead of returning: resolution failure → the resolver's own reason, wrong kind → `entity_type_mismatch: '<type>'`. So a rejection is safe to retry and emits no change-feed rows for a non-existent entity. Per request, not an undo of prior ones — a re-send whose identifier landed earlier still returns `auto-attached`. The event-native `POST /orgs/{id}/events/observations` is exempt by design: partial-success, per-event savepoint (see §"Event observations")
...
- Timestamps: `datetime` + `@field_serializer` → `TimestampStr` via `fmt_ts()`; `Z`, six-digit fraction; **never format in a handler** — `fmt_ts` is serializer-only. Four sweeps in `tests/api/public/test_timestamp_serialization.py` (#440) cover the formatter ban, `str`-typed timestamp fields, serializer coverage, and the published OpenAPI shape. Full rules → `docs/CONVENTIONS.md` §"Public API"
...
- Assignment observations (#311): `(person, role, start_date)` is identity, not payload — PM-native (`pm_assignment_id`) observations **update in place** (`update_assignment_fields`: move start, explicit `end_date: null` clears, tri-state `is_current`); natural-key auto-attach applies only the open-tenure close, other deltas come back in `unapplied`. Provenance: `role_assignments.source_key_id` stamped on NEW, updates gated to same-or-NULL source; an **identical** id-addressed assertion against an **unowned** row claims it (#478, provenance-only UPDATE + `provenance_claimed: true` — an owned row, same-source or foreign, still takes no write from agreement). Dup cleanup: `scripts/audit_assignment_duplicates.py`. **Retract (#391):** `op="retract"` archives the `pm_assignment_id` tenure → new `retracted` disposition — the correction for a produced *artifact* (never happened), which closing can't express and un-producing only orphans. Always id-addressed (`invalid` otherwise); `person_id`/`role_id` must match (`identity_immutable`); same-or-NULL provenance (`source_key_mismatch`); payload + ancillary ignored; re-emit is a quiet `auto-attached` no-op (lookup deliberately **not** archived-filtered, unlike `update_assignment_fields`, and not routed through `resolve_entity`) — carrying `attached_archived: true` (#477), the only field that says the addressed row is retracted. **Authoritative** — both create doors skip an archived twin instead of resurrecting (`resolve_assignment` + the embedded `write_role_assignments`), so a re-emitting producer can't defeat the retract; that attach **writes nothing** (bound deltas *and* ancillary withheld, every name echoed in `unapplied` — reported even when the value equals what the archived row stores, unlike the active path); un-retract = admin unarchive only, no `archived:false` verb. Archiving cascades free: outbox emit + `staff_of` edges archive (#301). Full rules → `docs/API_ASSIGNMENTS.md` §"Write semantics & provenance".
- Observation atomicity (#456): resolution + payload share one transaction on the four identity-observation endpoints, so `rejected` = nothing written, entity included; the resolve and entity-kind guards raise inside the transaction rather than returning. The event-native endpoint is partial-success and exempt. Full rules → `docs/CONVENTIONS.md` §"Public API"
