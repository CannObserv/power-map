...
- **Merge re-homing:** every merge that collapses a citable entity re-homes its citations onto the survivor **before** the delete (`migrate_citations`/`rehome_citations`, NULL-safe active-scoped dedup) — wired into `rehome_conflicting_assignment_ancillary` (role_assignment), `rehome_role_ancillary` (role), `delete_role_ancillary` (drop), and the primary person/org DELETE in `people_merge`/`orgs_merge`. Citations self-emit the survivor signal via the touch trigger. **Known gap (audit-guarded):** `person_name`/`entity_event` citations stranded by sub-entity dedup during a merge are caught by the daily orphan audit (`count_orphaned_citations`, `citation.<type>` scope) rather than proactively re-homed — a follow-up.
