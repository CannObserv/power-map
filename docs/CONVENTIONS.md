...
**Every path that can strand a person without a display pointer must repair it**, and they all call the same helper — `heal_person_canonical`: the observation path (including observations carrying no names), name deletion (`_maybe_promote_sole_name`, which is now a thin delegation), merge (`merge_person_into`, which demotes the loser's canonical and so must heal the winner), and the `#308c` backfill, which shares the ladder via `name_type_priority_sql()`. One implementation means every route picks the same replacement row.

Do not reintroduce a "promote only when exactly one name remains" shortcut. That was the old delete-path rule, and it left a multi-name person blank whenever their canonical was deleted — the remaining names were perfectly displayable, nothing repaired them, and only a later observation happened to fix it.
