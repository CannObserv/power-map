...
The heal is a **single statement with no savepoint**: its guard is keyed on the unique-index key `(person_id, name_type, COALESCE(locale,''), COALESCE(script,''))` rather than a person-wide check, so `uq_person_canonical_name` cannot reject it. (A savepoint costs two extra round trips — ~25 ms per observation — to guard an exception that keying the guard correctly makes impossible.)
...
- `scripts/backfill_person_canonical_names.py` excludes them from `find_candidates` and reports them in a separate `blocked` bucket. This exclusion is load-bearing: the backfill promotes inside one savepoint, so a single such person raising `UniqueViolationError` would roll back every other promotion in the run.
