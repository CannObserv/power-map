...
### Event observations — refine-in-place, partial-success & `succeeded_by` (#321)
...
- **Follow-up (not in #321):** immutable identity means a mis-linked dateless event (`succeeded_by`/`split_from`/`merged_with` — no mutable field to refine) can only be corrected by create-new + retract-old; an event void/retract op is the tracked follow-up.
