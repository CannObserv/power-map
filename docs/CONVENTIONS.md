...
- **Seat titles are PM-curated (#267):** `title` is not part of a seat's identity (matching keys on the tuple), so a seat observation may omit it — on create `resolve_role` synthesizes the canonical title from the tuple via `src.core.seat_title` (the single formatter, shared with `scripts/generate_wa_seats.py`) and **prefers it over any supplied title**, so upstream observers can't drift PM's curated form. A supplied title is used only as a fallback when the seat can't be synthesized; a titleless seat that can't be synthesized (unknown `role_type` / non-`usa-wa-ld` district) is rejected `seat_title_unavailable`.
- **`role_types` catalog is publicly observable (#268):** `GET /api/v1/role-types` returns the classifier vocabulary (`id, slug, display_name, is_seat`) so producers discover the seat-match `role_type` slugs instead of hardcoding them. `is_seat` is an **advisory** hint that the office is normally a districted seat — `resolve_role` does not enforce it. An unknown `role_type` slug is already rejected (`role_type_not_found`), so the endpoint's role is to prevent a *valid-but-wrong* slug (which would mint a duplicate seat).
