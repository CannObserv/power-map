...
- Timestamps: use `datetime` fields in Pydantic response models + `@field_serializer` calling `fmt_ts()` from `schemas.py`; ISO 8601 with `Z` suffix; never pre-serialize as `str` in handlers. Two AST sweeps in `tests/api/public/test_timestamp_serialization.py` enforce it (#440). **(1) No pre-serialization** — `.isoformat()` anywhere in `src/api/public/*.py` outside `fmt_ts` itself fails (only `<dt>.date().isoformat()` is exempt: a calendar date has no offset), as does a `str`-typed `…_at` field on a response model. This is the load-bearing half: pydantic ≥2.12 already emits `Z` for a zero-offset aware datetime, so a hand-built string is the *only* way a `+00:00` reaches a consumer, and it bypasses the response model where no serializer can catch it. **(2) Every response-model `datetime` field carries an `fmt_ts` serializer** — redundant with pydantic's default today, which is exactly why it rots unnoticed; it is the declared insurance against a pydantic default change or a non-UTC-aware datetime reaching a model. A serializer that hand-rolls `.replace("+00:00", "Z")` does not count — the point is one formatter, changeable in one place. **Request models are exempt** and are excluded by *resolution*, not by name: the sweep walks bases and field annotations out from every `response_model=` declaration, so anything unreachable is parsed-only (a name heuristic would misclassify `CitationObservationItem`, a request model). A serializer's return annotation should match its field's optionality — `-> str | None` on a required field publishes it as nullable in OpenAPI. Both sweeps are ratchets, not proofs: indirection through a helper, an f-string or `str()` slips past, same caveat as the conditional-GET sweeps. Sibling repo `observo` carries the byte-identical convention and drifted (its #450, ~12 hand-built dict payloads on `+00:00`) — the guard is what keeps one enforceable policy across both
...
- Timestamps: `datetime` fields in response models + `@field_serializer` calling `fmt_ts()` from `schemas.py`; ISO 8601 with `Z` suffix; never pre-serialize as `str` in handlers. Two sweeps in `tests/api/public/test_timestamp_serialization.py` enforce both halves — no `.isoformat()` outside `fmt_ts`, and an `fmt_ts` serializer on every `datetime` field reachable from a `response_model=` (request models exempt by reachability, not by name) (#440). Full rules → `docs/CONVENTIONS.md` §"Public API"
