...
**Every path that can strand a person without a display pointer must repair it.** Four do: the observation path and nameless observations (`heal_person_canonical`), name deletion (`_maybe_promote_sole_name`), the `#308c` backfill, and **merge** (`merge_person_into`, which demotes the loser's canonical and so must heal the winner). `heal_person_canonical` is the shared implementation and is a no-op when the person already displays; it uses the same `name_type_priority_sql()` ladder as the backfill, so the two repair paths always choose the same row. It is best-effort: a failed promotion is swallowed (any `PostgresError`, inside a savepoint) because aborting an observation over a cosmetic display-name repair would discard its links, addresses, role assignments and events.
