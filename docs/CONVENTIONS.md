...
- Timestamps: `datetime` fields in Pydantic response models + `@field_serializer` calling `fmt_ts()` from `schemas.py`; ISO 8601 with `Z`; never pre-serialize as `str` in a handler. Three sweeps in `tests/api/public/test_timestamp_serialization.py` enforce it (#440) — two over the source AST, one over the live `app.openapi()`; that module's docstring carries the reasoning and the ratchet caveats, so read it before changing any of this. **(1)** No `.isoformat()` in `src/api/public/*.py` outside `fmt_ts` (only `<dt>.date().isoformat()` is exempt), and no `str`-typed `…_at` field on a response model — a hand-built string is the only way a `+00:00` reaches a consumer, since pydantic already emits `Z` for a zero-offset aware datetime. **(2)** Every `datetime` field reachable from a `response_model=` declaration carries an `fmt_ts` serializer; hand-rolling `.replace("+00:00", "Z")` does not count. Request models are exempt **by reachability, never by name** (`CitationObservationItem` is a request model). **(3)** The serializer's *return* annotation is what OpenAPI publishes, so it is `TimestampStr` (`Annotated[str, …]` carrying `format: date-time`, which a bare `-> str` erases) and its nullability must match the field's — a serializer spanning required *and* optional fields is split in two. `fmt_ts` normalizes nothing: a naive or `+05:00` datetime passes through unchanged, so the single formatter is where to add teeth if that ever matters. Sibling repo `observo` carries the byte-identical convention and drifted (its #450)
...
- Timestamps: `datetime` fields in response models + `@field_serializer` calling `fmt_ts()` from `schemas.py`; ISO 8601 with `Z` suffix; never pre-serialize as `str` in handlers. Three sweeps in `tests/api/public/test_timestamp_serialization.py` enforce it — no `.isoformat()` outside `fmt_ts`; an `fmt_ts` serializer on every `datetime` field reachable from a `response_model=` (request models exempt by reachability, not by name); and a return annotation of `TimestampStr`/`TimestampStr | None` matching the field, since that is what OpenAPI publishes (#440). Full rules → `docs/CONVENTIONS.md` §"Public API"
