...
migrations and audits in `docs/RUNBOOKS.md`.
...
| Per-key API anomaly check | hourly `power-map-anomaly.timer` runs `scripts/check_api_anomalies.py` — journal WARNING + exit 3 per key ≥ `API_ANOMALY_HOURLY_THRESHOLD` req/hr (#294); human layer = Admin → Activity → API Requests per-key panel; see `docs/RUNBOOKS.md` |
| Schema-parity audit | daily `power-map-schema-parity.timer` runs `scripts/audit_schema_constraint_parity.py` — snapshots full `pg_get_constraintdef` + `pg_get_functiondef` + `pg_get_triggerdef` on reference (`PARITY_REFERENCE_URL`, default `TEST_DATABASE_URL`) vs prod, exit 3 on any missing/different object, per-kind breakdown `constraint.*`/`function.*`/`trigger.*` (#315 constraints + #331 functions/triggers; `CREATE TABLE IF NOT EXISTS` inline-drift + `CREATE OR REPLACE` body-drift; extension-owned/internal excluded; function/trigger diff skipped on a PG-major mismatch); see `docs/RUNBOOKS.md` |
...
| Egress-IP drift guard | every 5 min `power-map-egress-ip.timer` runs `scripts/check_egress_ip.py` — compares this host's public egress IP against `EGRESS_EXPECTED_IPS` (`/etc/power-map/.env`), exit 3 + `egress-ip-drift` GH issue carrying the **new address** on mismatch. The DO cluster gates on source IP and the exe.dev egress IP is NAT'd and unpinned, so a rotation kills every DB-backed route (2026-08-09, #410). Losing every lookup service is **not** drift — WARNING, exit 0. Keep `EGRESS_EXPECTED_IPS` in step with terraform's `allowed_external_ips` (#409). Triage → `docs/RUNBOOKS.md` § Database unreachable. Hatches: `EGRESS_CHECK_NO_GH=1`, `EGRESS_CHECK_FORCE_FAIL=1` |
