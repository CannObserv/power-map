...
| usa-wa dataset pull | nightly 09:00 UTC `power-map-datasets-pull.timer` runs `scripts/pull_datasets.py` — fetches usa-wa's catalog over an exe.dev VM bearer token (`USA_WA_TOKEN`), lands every subscribed dataset whose latest version is not already held into `data/usa_wa_snapshots/`, verifies each `data.csv` against the digest the catalog states, and prunes to the newest `--keep` versions (never the one just landed). Writes **no** database rows — the gated step is the applier (#499). Exit 1 on a failed digest, an incompatible schema major, or a subscribed dataset the catalog does not carry. Runs an hour after usa-wa publishes at 08:00 UTC; see `docs/RUNBOOKS.md` |
