...
- **+ Add name guard**: the detail page's "+ Add name" button (`id="add-name-btn"`) is disabled while `#name-row-new` is on the page (an unsaved new-name row exists). Wired by `src/static/admin/person-detail-add-name-guard.js`: listener on `#names-table` (not `document.body`) catches `htmx:afterSwap` from Save / Edit-Cancel; a custom `powerMap:newNameRowClosed` event catches the new-name form's inline Cancel (which removes the row directly without an HTMX round-trip). Saving or cancelling re-enables the button.
- **`powerMap:` custom event prefix**: project-wide convention for client-side custom DOM events that don't go through HTMX's `HX-Trigger` header (those follow the `update{Entity}Header` camelCase shape — see [§JS file](#js-file-srcstaticadminentity-detailjs)). Today's only `powerMap:` event is `powerMap:newNameRowClosed`; future custom events use the same prefix to avoid colliding with browser/library events.
