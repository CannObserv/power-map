...
- **Typeahead query parameter**: locale / script / reading-of inputs must send `q={value}` to their search endpoints. The display inputs are unnamed (so they don't pollute the parent Save POST) and use `hx-vals='js:{q: event.target.value}'` to set the request param. The earlier shape (`name="q_locale"` + `hx-params="q"`) silently sent no `q` because no input was named `q` — the filter dropped everything. Don't reintroduce `hx-params="q"` here.
- **Per-row ID namespacing**: every typeahead element id (`locale-search-display`, `locale-search-results`, `locale-hidden`, the script + reading-of equivalents, and `reading-of-block`) is suffixed with a `_uid` discriminator. `_name_metadata_fields.html` and the inline init `<script>` in `_name_form_row.html` agree on `{%- set _uid = n.id if n else 'new' -%}`. This lets an open Edit drawer and the inline new-name form coexist on the page without `getElementById` collisions. The reading-of toggle's `select[name="name_type"]` lookup is also row-scoped (`document.getElementById('name-row-{n.id|new}').querySelector(...)`), not document-global.
- **+ Add name guard**: the detail page's "+ Add name" button is disabled while `#name-row-new` is on the page (an unsaved new-name row exists). Clicking again would otherwise prepend a second `<tr id="name-row-new">`, reintroducing the same id-collision class as before namespacing. Saving or cancelling the row removes the `name-row-new` element and re-enables the button (the unified tbody re-render restores the ready state).
