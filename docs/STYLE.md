...
Accessibility rules and their test tiers live in [`docs/ACCESSIBILITY.md`](ACCESSIBILITY.md).
...
- `id="page-heading"` — required when the page title can change via HTMX (e.g. after an inline name edit). `org-detail.js` listens for `updateOrgHeader` events and updates `#page-heading`, `#breadcrumb-current`, and `document.title` in-place. See §32 → Page header sync.
...
- Flash on detail pages: `org_detail`, `person_detail`, `ra_detail` accept `?flash=` param via `resolve_query_flash`; add entity-specific flash keys to the module-level `_FLASH_MESSAGES` dict. `resolve_query_flash` also falls back to `SHARED_FLASH_MESSAGES` (`saved`/`removed`/`invalid`/`exists`) — the generic §32 ancillary-fallback keys (#351) — so a target route need not re-declare them; a route-local key of the same name still wins
