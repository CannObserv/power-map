...
- **Jurisdiction typeahead**: `GET /admin/jurisdictions/search/` (`src.api.admin.jurisdictions`) is a read-only `<li role="option" data-id data-label>` fragment for the shared `typeahead-combobox.js` factory. Mirrors `/admin/orgs/search/`. (Jurisdictions also have a browse surface — see "Jurisdictions — browse surface" below.)
...
### Jurisdictions — browse surface (#275, Phase 1)

`src.api.admin.jurisdictions` surfaces jurisdictions as a first-class browsable entity (sidebar link, Entities-index card, dashboard count):

- **List** (`GET /admin/jurisdictions/`) via `query_jurisdictions_rows` (`jurisdictions_queries.py`). Search is **ILIKE on name/slug** — jurisdictions have no `search_tsv` (unlike orgs/people, which use `plainto_tsquery`). Type filter from `jurisdiction_types`; three-value status axis **active / superseded / archived** partitioning on `(archived_at, superseded_at)` (a superseded row keeps `archived_at IS NULL` — supersession is not soft-delete). HTMX region swap mirrors orgs.
- **Read-only detail** (`GET /admin/jurisdictions/{id}/`): core fields + read-only panels for identifiers, links, addresses, contacts, plus **relationships** (both directions, typed/dated), **lineage**, **affiliated organizations**, and **referencing roles** (`roles.jurisdiction_id`, archived excluded — the reciprocal of the role-form picker). CRUD (create/edit/archive + interactive attachment/graph editing) is deferred to Phases 2–3; panels are read-only lists for now.
