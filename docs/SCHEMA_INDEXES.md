...
- **Titles of roles with a jurisdiction are not identity, but are required (#497):** `title` is not part of such a role's identity (matching keys on the tuple) and is stored as supplied. #267 had `resolve_role` synthesize a canonical title and prefer it over the observer's; the #490 design retracts that — the producer owns the title, PM's overrides live in the curation overlay. (Formerly: upstream observers can't drift PM's curated form. A supplied title is used only as a fallback when it can't be synthesized; a titleless role that can't be synthesized (unknown `role_type` / non-`usa-wa-ld` district) is rejected `role_title_unavailable`.
