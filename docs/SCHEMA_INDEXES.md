...
- **Titles of roles with a jurisdiction are not identity, but are required (#497):** `title` is not part of such a role's identity (matching keys on the tuple) and is stored as supplied; a titleless create is rejected `title_required`, the same reason a plain role has always got. #267 had `resolve_role` synthesize a canonical title, prefer it over the observer's, and reject an unsynthesizable titleless role `role_title_unavailable`; the #490 design retracts all of that — the producer owns the title, PM's overrides live in the curation overlay, and `role_title_unavailable` no longer fires.
...
**Backfill-before-trigger (#392).** A reconciliation block that *backfills a
column* must appear **before** any `set_updated_at()` trigger on that table.
File order is execution order, and the trigger overwrites `NEW.updated_at` with
`NOW()` — so a backfill placed after it is silently clobbered on every row it
touches. Fresh-DB tests cannot catch this: on a fresh DB the backfill has no rows
to fix, so the ordering never matters and the guard passes. Moved here from
`docs/SCHEMA.md` (#518), which stated it in an index line.
...
2. **Domain-prefix convention.** Every non-jurisdictional slug is prefixed by the org-kind it attaches to: `committee_`, `chamber_`, `legislature_`, `party_` (corporate/advocacy `org_*` is reserved for #303). The prefix disambiguates a bare noun that means different things per domain (a `member` on a committee vs a party) and yields coarse rollups for free (`WHERE slug LIKE 'legislature_%'`) — so you rarely need a separate coarse type. The rule reaches **non-jurisdictional slugs only** — jurisdictional seat types are unprefixed by scope, not by exemption, so a new one (`state_representative_at_large`, #302) is unprefixed for the same reason its siblings are. `state_representative` and `state_senator` are *additionally* grandfathered against renaming: they predate the convention, and renaming a public-API-visible slug (`GET /api/v1/role-types`) is breaking and buys nothing.
3. **Concept in the type, jurisdiction label in the title.** Types stay jurisdiction-neutral; the jurisdiction-specific label is the producer's, carried by the free-text `title` (#497 retired PM's renderer, `src/core/role_title.py`). Don't mint `wa_speaker` — the type is the concept, the label is the title.
...
**"On first observation" means a human seeds it, not the observation.** `role_types` has **no remote write path** — the seed `INSERT … ON CONFLICT` in `src/core/schema.sql` is the only writer in the codebase; the public endpoint is GET-only and the admin role-type control is a `<select>` over existing rows. `resolve_role` rejects an unknown slug outright (`role_type_not_found`), so an observation of an unseeded type does not create it — it bounces, silently, as a disposition rather than an error. Seed the row and deploy **before** a producer starts emitting it.

**At-large / block-voting multi-member seats (#302).** Where a chamber fills N *fungible, individually-undesignated* seats per district from one candidate pool (WA House 1889–1965, before Ch. 52 Laws of 1965 introduced Position 1 / Position 2), the seat is its own **`state_representative_at_large`** role type — `expects_jurisdiction=TRUE`, `requires_qualifier=FALSE`, `forbids_qualifier=TRUE`. Seeded ahead of the data (against rule 1's default) precisely because of the paragraph above: usa-wa's pre-1965 records are imminent and every one of them would otherwise be rejected. Three rules follow:

- **Multiplicity is assignments, not rows.** One at-large role per (chamber, district) with `qualifier` NULL, and N **concurrent `role_assignments`** on it. Fungible means un-individuated: the seat is not the identity, the membership is. `uq_role_assignment_person_role_start` keys on `(person_id, role_id, start_date)` and no overlap constraint exists, so concurrent occupancy is already legal. Never mint `Position 1`/`Position 2` (or an "at-large seat k of N" qualifier) for an at-large era — both fabricate a designation that legally did not exist and force an arbitrary person→seat pick.
- **The NULL qualifier is unambiguous by construction.** "At-large, no position ever existed" cannot be confused with "positioned seat, position unknown" because `requires_qualifier=TRUE` + `trg_role_requires_qualifier` make a NULL-qualifier `state_representative` unrepresentable on every path (#273). The data-gap shape does not exist to collide with.
- **`forbids_qualifier` is the symmetric guard, and it is load-bearing.** `requires_qualifier=FALSE` only *permits* a NULL qualifier — on its own it does not forbid a value, so a producer's stray `qualifier` would mint a **second** role for the district titled `"… (At-Large), LD-N, Position 1"`: the same silent spurious-seat failure #267/#273 close from the other direction. `forbids_qualifier=TRUE` rejects it as `qualifier_forbidden` in `resolve_role`, with `trg_role_forbids_qualifier` backstopping admin and direct-INSERT paths. The two flags are mutually exclusive (`chk_role_type_qualifier_policy`); both FALSE means the qualifier is optional (`state_senator`). Blank/whitespace is absence on both paths and normalizes to NULL.
- **Eras coexist on one district row via the roles' own lifespan.** At-large role `abolished_on` = the designation cutover; the positioned roles' `established_on` = the same date. `uq_role_structural` has no temporal predicate, but it keys on `role_type_id`, so the at-large role's NULL qualifier only has to be unique *within its own type* — all three roles (at-large, Position 1, Position 2) coexist on a single `jurisdictions` row without colliding.

**Accepted cost: district identity is conflated across redistricting.** usa-wa attaches pre-1965 tenures to the **current** `usa-wa-ld-N` rows, so 1901's LD-5 and today's LD-5 are one row — even though the 1965 redistricting that created Positions also redrew the map. This is a deliberate call, not an oversight: jurisdictions carry no geometry (they are identity + name + type + validity window + graph edges), so no boundary fidelity is lost — what is lost is the ability to say *which* LD-5 a historical role belonged to. Nothing enforces this either way; a consumer inferring district continuity from a shared `jurisdiction_id` will be wrong for pre-1965 rows. #448 tracks per-era jurisdiction rows if that ever needs to be recovered.

Seat **cardinality** ("this office has N indistinct seats") is deliberately not modelled: N varies by district and era, so it is not a property of the type. If a consumer ever needs the established count it belongs as a nullable `roles.seat_count` on the row.

**Current catalog (post-#266, post-#302):** `state_representative`, `state_senator`, `state_representative_at_large` (jurisdictional seats — unprefixed by rule 2's scope; the first two also grandfathered against rename); `chamber_leader`, `chamber_officer` (coarse); `committee_chair`, `committee_vice_chair`, `committee_ranking_member`, `committee_assistant_ranking_member`, `committee_member`; `legislature_staff` (coarse); `party_member` (coarse). The pre-#266 coarse `member` was split into `committee_member` + `party_member` and dropped.
