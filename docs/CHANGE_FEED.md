...
| `person` / `organization` / `jurisdiction` / `role` / `role_assignment` | `deleted_entities` | `change_kind` is always `"deleted"`. `role` / `role_assignment` deletions began emitting tombstones in #277; the ones a **merge** performs, in #467 |
...
- **Merges preserve ids wherever the schema allows (#467).** Folding one org, person or role into another **re-points** its role assignments — they keep their ULIDs, so a `pm_assignment_id` anchor survives the merge and needs no repair. Only a genuine `(person_id, role_id, start_date)` collision, which `uq_role_assignment_person_role_start` cannot hold twice, is resolved by deleting the loser row; that row is tombstoned with `merged_into` naming the survivor. The loser role itself is always deleted (its assignments moved out) and always tombstoned. A subscription anchored to any deleted id gains the survivor alongside it at merge time — added, never moved, because the feed resolves subscriptions when you poll: dropping the loser's subscription would hide the loser's own tombstone from the only key that needed it.

  Two consequences to build for. **The survivor emits nothing on the merge itself** when the absorbed row carried no ancillary — no column of the survivor changed, so no outbox row is written for it. Act on the tombstone's `merged_into` by fetching the survivor from its read endpoint; do not wait for a feed event that is not coming. **And the stale anchor is yours to retire** — `DELETE /api/v1/subscriptions/{entity_id}`, or the bulk `DELETE /api/v1/subscriptions`. Nothing prunes subscriptions server-side, so the set grows by one row per merge and stays that way; the 90-day tombstone TTL is the window in which a stale row still explains itself, after which it is an id resolving to nothing with no record of why. Before #467 the subtree was migrated by re-creating rows under fresh ULIDs with no tombstone at all: the data survived, every anchor broke, and the feed said nothing.
