...
> **Implemented (post-CR revision).** The original task was bundled with the structured-parts decision; both ship together now. The historical RED-state snippet is dropped — see the actual implementation:
>
> - Schema: `src/core/schema.sql` (per-column DO blocks for `locale`, `script`, `sort_as`, `visibility`, `reading_of_id`)
> - Parts sidecar: `src/core/schema.sql` `person_name_parts` table
> - Tests: `tests/core/test_schema_person_names_i18n.py`
> - Commit: `f5fb7f3 #121 feat: add i18n columns to person_names …` (and the CR-followup `0574b9e` for the structural change)
>
> Adds `locale`, `script`, `sort_as`, `visibility`, `reading_of_id` directly on `person_names`. Structured parts (`given_names`, `family_names`, `additional_names`, `honorific_prefix`, `honorific_suffix`, `primary_identifier`) live in the new `person_name_parts` sidecar (1:0..1, ON DELETE CASCADE). All optional except `visibility` (default `'public'`).
