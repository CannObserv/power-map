...
⁴ **`using-mayfly-chat` (#564)** — live agent-to-agent exchange over a [Mayfly Chat](https://mayfly.chat) channel; needs Node.js ≥18 (the wrapper exits `4` without it). The channel URL is read, write **and delete** access: never commit it — not in an issue, commit, doc or plan. `tests/test_no_channel_urls.py` (ported from upstream, skills#302) reds on a live URL (id plus `#key`) in any tracked file; run the skill's `references/security.md` leak check over untracked output before committing it. `tests/test_skills_listing.py` keeps this table and `.claude/skills/` in step — the auto-refresh hook bumps the pointer but never adds a link.
