...
An org's lifespan end is **derived, not a column**: `v_org_lifespan(organization_id, ended_on)` takes the earliest non-archived `dissolved` / `merged_with` / **dated `succeeded_by`** (#469 — the row is one source-keyed manifestation; a dated succession ends it while the successor row carries on) entity event, resolved to the *latest* date within the event's known precision (year-only 2023 → `2023-12-31`; month-only → last day of month) so closing an assignment at `ended_on` never claims an earlier end than the source supports. `renamed` / `split_from` imply continuity, not an end; an end event without a year (`merged_with` and `succeeded_by` don't require one) derives no bound. `organizations.active` is dateless state and `archived_at` is admin bookkeeping — neither is a lifespan; an org marked inactive **should** also get an end event when the date is known.
...
## Anti-resurrection is labelled — `attached_archived` (#477)


All four retractable observation paths — assignments (#391), events (#322), citations (#319), relationships (#301) — fold **two different outcomes** into the disposition `auto-attached`: a healthy attach to a **live** row, and an anti-resurrection attach to a **retracted** one. The producer needs to act on them oppositely (keep syncing vs. stop emitting this claim), and the disposition alone cannot tell them apart. That ambiguity is not theoretical: usa-wa stayed anchored to an archived assignment for a month, re-emitting it nightly, and found out only by hand-auditing the change feed (#474).

**The rule: any `auto-attached` that addressed an archived row carries `attached_archived: true`.** It is an **additive optional field**, `null`/absent everywhere else — never `false` on a healthy response, so the common case stays quiet and no consumer has to change to keep working.

- **Where it appears.** `ObservationResponse` (assignments — both the standard-mode archived-twin attach and a re-emitted `op="retract"`), and the per-item results `EventObservationResult` / `CitationObservationResult` / `RelationshipObservationResult`, on **both** transports each (embedded in `ObservationResponse.events` / `.citations`, and the native partial-success endpoints).
- **Both archived branches are labelled**, not just the dedup one: the anti-resurrection match *and* the already-archived `op="retract"` no-op. Both mean "the row you are addressing is retracted"; distinguishing them is the producer's job via the disposition it sent.
- **Not a new `disposition` value.** `auto-attached` is a parsed contract string for every consumer; widening the enum would break parsers to fix an observability gap. The signal rides alongside it.
- **Distinct from `unapplied` (#311).** `unapplied` says *what* was withheld and is populated on the active path too; it never meant "archived". A producer inferring retraction from the presence of a field name in `unapplied` is reading a signal that was never promised — `attached_archived` is the one that is.
- **Carried by the core result types**, not assembled at the route: `AssignmentResolution`, `EventResult`, `CitationResult`, `RelationshipResult` each expose `attached_archived: bool` (default `False`), and every wire mapping forwards it — ratcheted by `tests/api/public/test_attached_archived_sweep.py`, which fails on a construction site that drops it.
- **Not covered:** the embedded `role_assignments: [...]` array on a people observation. Its archived-twin skip is real but that transport returns no per-item results at all, so there is no slot to carry the flag — a producer needing the signal must use `POST /assignments/observations`.
- **Naming the live sibling** (the id a producer should re-anchor to) is deliberately *not* part of this — it needs a "which sibling" rule of its own.
...
- **The gate scopes this API, not the column (#499).** The dataset applier writes `organizations.parent_id` directly — the ownership manifest gives the producer that column — so it neither consults `source_key_id` nor claims it. Row scope there is `producer_crosswalk`, and column scope the manifest; a `source_key_id` an observation left behind neither blocks nor survives an applier reparent. The four orgs carrying the column are exactly the four the applier's first production run wanted to reparent.
...
- **Observation semantics** (`src/core/assignment_relationships.py`, mirrors events/citations): pm-native only — each claim references its endpoints by `pm_assignment_id`. Natural-key observe (`from`+`to`+`rel_type`) → refine matched active row / anti-resurrect archived twin / create; `pm_relationship_id` → id-addressed refine (identity immutable → `identity_immutable`); `op="retract"` archives (already-archived no-op). Both archived outcomes carry `attached_archived: true` (#477). Diff-before-write precedes the `source_key_id` same-or-NULL gate (`provenance_conflict`). Reason slugs incl. transient `assignment_unresolved`, plus `rel_type_unknown`/`self_relationship`/`relationship_not_found`.
