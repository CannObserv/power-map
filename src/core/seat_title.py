"""Canonical WA legislative seat-title synthesis (#267).

Single source of truth for the human-readable title of a WA legislative
seat-Role, shared by the seed generator (``scripts/generate_wa_seats.py``) and
the observation resolver (``src.core.observation.resolve_role``). Centralizing
the format here means PM curates a seat's title deterministically from its
structural tuple ``(role_type, jurisdiction, qualifier)`` — an upstream observer
can omit ``title`` entirely and never nudge PM's curated form.

Scope: WA legislative seats only. A seat whose ``role_type`` slug is unknown, or
...
def wa_legislative_seat_title(
    role_type_slug: str, ld_number: int | None, qualifier: str | None
) -> str | None:
    """Render a WA legislative seat title, or ``None`` if it can't be synthesized.
    ...
    """
    base = _WA_SEAT_TITLE_BASE.get(role_type_slug)
...
def synthesize_seat_title(
    role_type_slug: str, jurisdiction_slug: str | None, qualifier: str | None
) -> str | None:
    """Synthesize a seat title from the role_type slug + jurisdiction slug.
    ...
    """
    return wa_legislative_seat_title(
