...
def _markdown(summary: dict) -> str:
    ...
    if producer_stale(summary.get("build_info")):
        ...
        lines += [
            "",
            f"**usa-wa was behind its clock when this was built:** it last completed a run"
...
def may_execute(ledger: Sequence[dict], *, digest: str, streak: int) -> tuple[bool, str]:
    ...
    for ln in recent:
        ...
        # A frozen input and a settled diff look identical from inside a run, and
        # a stable digest is the whole evidence this gate consumes (#551). Read
        # with `.get`: every line written before this existed carries no key, and
        # a deploy must not restart a streak the nightly has been building.
        if ln.get("producer_stale"):
            return False, (
