...
- Person **canonical/display** name (#308): `is_canonical` is per `(person_id, name_type, locale, script)`, so a person may hold several canonical rows — `v_person_display_names` disambiguates with `DISTINCT ON (person_id)` + a `name_type` priority ladder. That ladder is mirrored by `_PERSON_NAME_TYPE_PRIORITY` in `src.core.observation`; **adding a `name_type` means updating both**, or it sorts last by default (`tests/core/test_schema_person_display_view.py` asserts the two agree). `write_names` auto-promotes one name per write when the client sends no `is_canonical` hint, and heals a canonical-less person on re-observation; `NO_AUTO_CANONICAL_NAME_TYPES` (deadname/mrz/romanization/reading) is never auto-promoted, on both the observation and admin-delete paths. A canonical slot already held by a **non-public** row is never displaced — PM logs a WARNING and the backfill reports a `blocked` bucket. Full rules → `docs/CONVENTIONS.md` §"Canonical auto-promotion on observation".
