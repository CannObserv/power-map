...
- Archive model: `archived_at TIMESTAMPTZ` — NULL = active, non-NULL = archived; hard delete gated on `archived_at IS NOT NULL` (returns 409 if not archived); unarchive via `POST /{id}/unarchive/` sets `archived_at = NULL` and preserves prior `active` state (returns 409 if not archived). Orgs/people/roles archive routes are idempotent (re-archive silently updates `archived_at = NOW()`); role-assignments archive is strictly guarded and returns 409 if already archived (both inline routes on role/person detail and the RA detail page).
