...
| role / role_assignment ancillary orphan audit | daily `power-map-ancillary-orphans.timer` runs `scripts/audit_ancillary_orphans.py` — anti-join count of no-FK polymorphic ancillary keyed on a non-existent parent, over **both** scopes: `role_assignment` (`links`/`contact_methods`/`field_confidence`/`identifiers`, #324) and `role` (`links`/`contact_methods`, #326); exit 3 on any orphan, breakdown namespaced `role.*`/`role_assignment.*`. Recovery: `scripts/cleanup_role_assignment_ancillary_orphans.py` (heuristic re-home, dry-run → `--execute`; role_assignment-only — role orphans go to manual triage); see `docs/COMMANDS.md` |
