...
- Person display names: use `v_person_display_names` (view in `schema.sql`) for admin queries that display a person name — returns the canonical `person_names` row. Currently used by duplicate detection; future list/detail queries should use this view rather than joining `person_names` directly.
...
- Notes inline edit pattern: GET `/inline/notes/` → read partial; GET `/inline/notes/edit/` → form partial; POST `/inline/notes/` → read partial. Both target `id="notes-field"`. Form partial: `<form>` wraps the whole partial; header row (`display:flex; justify-content:space-between`) holds `<label for="notes-textarea">` (not `<h3>`) on the left and Save/Cancel buttons on the right — same structure as the read partial's `[Notes h3] [Edit]` header, so no layout shift on toggle. No `form-actions` div. Save `notes.strip() or None` (whitespace → NULL). See `docs/STYLE.md §15` for full pattern.
...
- Child org search uses a scoped endpoint `GET /{org_id}/children/search/?q=...` (not the generic `/search/`) — excludes the current org and orgs already linked as children (`parent_id = org_id`). Use the scoped endpoint for any context where existing relationships must be filtered from typeahead results.
