...
| Schema constraint-parity audit | daily `power-map-schema-parity.timer` runs `scripts/audit_schema_constraint_parity.py` — snapshots full `pg_get_constraintdef` on reference (`PARITY_REFERENCE_URL`, default `TEST_DATABASE_URL`) vs prod, exit 3 on any missing/different constraint (#315; the `CREATE TABLE IF NOT EXISTS` inline-drift class, FK actions included); see `docs/COMMANDS.md` |
| role_assignment ancillary orphan audit | daily `power-map-ancillary-orphans.timer` runs `scripts/audit_role_assignment_ancillary_orphans.py` — anti-join count of `links`/`contact_methods`/`field_confidence`/`identifiers` rows keyed on a non-existent role_assignment id, exit 3 on any orphan (#324; the no-FK merge-dedup orphan class). Recovery: `scripts/cleanup_role_assignment_ancillary_orphans.py` (heuristic re-home, dry-run → `--execute`); see `docs/COMMANDS.md` |
