...
- OOB flash in table-row partials: `<div>` OOB elements cannot be adjacent to `<tr>` in a response — the browser's HTML parser strips the `<tr>` when HTMX places both into a container `<div>`. Wrap the OOB flash in `<template>` (HTMX 1.9.8+ scans inside `<template>` for `hx-swap-oob`): `{% if flash_level %}<template>{{ flash_oob(flash_level, flash_body) }}</template>{% endif %}`. See `_parent_read.html` as canonical example.
