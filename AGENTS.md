...
**Operational scripts — dry run by default (#402):** `DATABASE_URL` resolves to **production** from any directory, so a script in `scripts/` that writes must gate the write behind `--execute` and echo its target first. Both halves are load-bearing: the gate stops an unintended write, the echo makes an intended one attributable in scrollback. `scripts/_dsn.py` owns the redaction — `redact_dsn()` returns `None` for anything that is not a parseable URL and callers **never** fall back to printing the raw string (a libpq keyword/value DSN would hand `urlparse` the password as the "path"). `import_cannabis_observer.py` dry-runs by rolling back a real pipeline run, and its schema DDL is now opt-in `--apply-schema` (requires `--execute`); `seed_locales_scripts.py` previews insert/update counts. `apply-schema.sh` deliberately keeps its own copy of the redaction — an import failure on the `ExecStartPre` path would be a failed prod restart. #399 extends `_dsn.py` with prod/test labelling and retrofits it across the remaining live scripts.
