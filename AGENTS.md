...
- Conditional GET (#292/#392): all of it lives in `src/api/public/etag.py` — `make_etag` / `cache_headers` / `NOT_MODIFIED` / `conditional_response(request, response, etag, last_modified)`. A route calls `conditional_response` and returns its result when non-None; it **never reads the `if-none-match` header itself** (raw equality misses comma-lists, `W/` weak tags — GET compares weakly — and `*`; a comma inside a quoted tag isn't a separator). Sweep-enforced by `tests/api/public/test_conditional_get.py`. Two validator shapes only: watermark (`count(*)` + `max(updated_at)` over the visible set, filter params baked into the tag) where the table has `updated_at`, content-hash over fetched rows where it doesn't. New conditional GETs declare `responses=NOT_MODIFIED` + join `_CONDITIONAL_GETS` in `test_openapi_responses.py`. A 304 still costs a rate-limit token. Full rules → `docs/CONVENTIONS.md` §"Public API"
