...
def test_build_address_normalizer_local_only_skips_the_external_service(monkeypatch):
    ...
    set — independent of validate_addresses — so local_only is the only lever
    that keeps a preview off the wire.
    """
    monkeypatch.setattr(
...
def test_build_address_normalizer_local_only_overrides_validate_addresses(monkeypatch):
...
def test_build_address_normalizer_uses_the_service_when_not_local_only(monkeypatch):
    monkeypatch.setattr(
        "src.core.ingestion.pipeline.get_address_normalizer",
        lambda: FallbackAddressNormalizer(config=AddressNormalizerConfig(api_key="k")),
    )
