...
async def test_new_row_start_date_named_without_visible_label(client, role_id):
...
async def test_read_row_shows_note_indicator_when_notes_present(client, role_id, person_id, db):
    ra_id = generate_id()
    await db.execute(
    ...
    r = await client.get(
        f"/admin/roles/{role_id}/assignments/{ra_id}/read-row/", headers=HTMX_HEADERS
    )
    assert r.status_code == 200
    assert 'aria-label="Has notes"' in r.text


async def test_read_row_no_note_indicator_when_notes_absent(client, role_id, assignment_id):
    r = await client.get(
        f"/admin/roles/{role_id}/assignments/{assignment_id}/read-row/", headers=HTMX_HEADERS
    )
    assert r.status_code == 200
    assert 'aria-label="Has notes"' not in r.text


async def test_read_row_does_not_leak_note_text(client, role_id, person_id, db):
