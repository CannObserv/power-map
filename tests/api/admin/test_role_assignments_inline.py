...
def test_is_current_toggle_off(client, ra_id):
...
async def test_is_current_toggle_rejects_when_end_date_set(client, db, ra_id):
    ...
    assert "error" in response.headers["HX-Trigger"].lower() or \
           "end date" in response.headers["HX-Trigger"].lower()
    row = await db.fetchrow(
        "SELECT is_current FROM role_assignments WHERE id=$1", ra_id
    )
...
async def test_is_current_toggle_disabled_when_archived(client, db, ra_id):
    ...
    response = client.get(
        f"/admin/role-assignments/{ra_id}/", headers=AUTH_HEADERS
    )
    assert response.status_code == 200
...
def test_dates_read_partial(client, ra_id):
    response = client.get(
...
def test_dates_edit_partial(client, ra_id):
    response = client.get(
...
async def test_dates_post_updates_start_date(client, db, ra_id):
    await db.execute(
        "UPDATE role_assignments SET is_current=FALSE WHERE id=$1", ra_id
    )
    response = client.post(
    ...
    assert response.status_code == 200
    row = await db.fetchrow(
        "SELECT start_date, end_date FROM role_assignments WHERE id=$1", ra_id
    )
    assert str(row["start_date"]) == "2020-01-01"
    assert str(row["end_date"]) == "2021-12-31"


def test_dates_post_rejects_end_date_when_current(client, ra_id):
...
def test_notes_read_partial(client, ra_id):
    response = client.get(
...
def test_notes_edit_partial(client, ra_id):
    response = client.get(
...
async def test_notes_post_saves(client, db, ra_id):
    ...
    assert response.status_code == 200
    row = await db.fetchrow(
        "SELECT notes FROM role_assignments WHERE id=$1", ra_id
    )
...
def test_legacy_edit_page_removed(client, ra_id):
    """Full-page /edit/ form routes are gone."""
    response = client.get(
        f"/admin/role-assignments/{ra_id}/edit/", headers=AUTH_HEADERS
    )
    assert response.status_code == 404


def test_legacy_edit_post_removed(client, ra_id, person_id, role_id):
    response = client.post(
...
def test_detail_has_no_edit_link(client, ra_id):
