...
async def _fetch_link_type_id(db) -> str:
...
async def test_links_new_row_returns_form(client, org_and_link):
    oid, _ = org_and_link
    r = await client.get(f"/admin/orgs/{oid}/links/new-row/", headers=HTMX_HEADERS)
    assert r.status_code == 200
    assert "<form" in r.text


async def test_links_create(client, org_and_link, db):
    oid, _ = org_and_link
    lt_id = await _fetch_link_type_id(db)
    r = await client.post(
...
async def test_links_read_row_returns_row(client, org_and_link):
    ...
    r = await client.get(f"/admin/orgs/{oid}/links/{lid}/read-row/", headers=HTMX_HEADERS)
    assert r.status_code == 200
...
async def test_links_edit_row_returns_form(client, org_and_link):
    oid, lid = org_and_link
    r = await client.get(f"/admin/orgs/{oid}/links/{lid}/edit-row/", headers=HTMX_HEADERS)
    assert r.status_code == 200
    assert "<form" in r.text


async def test_links_update(client, org_and_link, db):
    oid, lid = org_and_link
    lt_id = await _fetch_link_type_id(db)
    r = await client.post(
...
async def test_links_delete(client, org_and_link):
    oid, lid = org_and_link
    r = await client.delete(f"/admin/orgs/{oid}/links/{lid}/", headers=HTMX_HEADERS)
    assert r.status_code == 200


async def test_links_delete_unknown_returns_404(client, org_and_link):
    oid, _ = org_and_link
    r = await client.delete(f"/admin/orgs/{oid}/links/{generate_id()}/", headers=HTMX_HEADERS)
...
async def test_links_create_returns_success_flash(client, org_and_link, db):
    oid, _ = org_and_link
    lt_id = await _fetch_link_type_id(db)
    r = await client.post(
        f"/admin/orgs/{oid}/links/",
...
async def test_links_update_returns_success_flash(client, org_and_link, db):
    oid, lid = org_and_link
    lt_id = await _fetch_link_type_id(db)
    r = await client.post(
        f"/admin/orgs/{oid}/links/{lid}/edit-row/",
...
async def test_links_create_duplicate_returns_409_not_500(client, org_and_link, db):
...
async def test_links_update_to_existing_returns_409_not_500(client, org_and_link, db):
