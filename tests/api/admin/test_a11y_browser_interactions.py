...
violation formatter, ``axe_check`` — plus the crash-resilient ``goto_with_retry``
(#436) from ``axe.py`` (#438). Same marker (``-m browser``) and isolation
constraints as the full-page sweep; run it alone against the dedicated test DB
(see docs/TESTING.md).
"""
...
async def _enter_people_merge_mode(page, live_server: str, seeded_ids: dict):
    ...
    state the merge-preview test continues from. Returns the page it drove: a
    renderer-crash retry (#436) replaces it, so callers must rebind.
    """
    page, _ = await goto_with_retry(page, live_server + "/admin/people/")
...
async def test_people_list_merge_mode_axe_clean(live_server, seeded_ids, page):
    ...
    page = await _enter_people_merge_mode(page, live_server, seeded_ids)
...
async def test_archived_person_delete_confirm_modal_axe_clean(live_server, seeded_ids, page):
    ...
    page, _ = await goto_with_retry(
        page, live_server + f"/admin/people/{seeded_ids['archived_person_id']}/"
    )
