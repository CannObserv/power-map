...
async def person_fixture(db):
    ...
    await db.execute(
        "DELETE FROM person_names WHERE id IN ($1,$2,$3)",
        canonical_name_id,
        variant_name_id,
        hidden_name_id,
    )
...
async def test_search_has_more_false_when_under_limit(client, api_key, person_fixture):
    r = _search(client, api_key, "Jane", limit=50)
...
async def test_get_person_by_id_full_record(client, api_key, person_fixture):
    ...
    # Only public names — canonical + variant, not hidden
...
async def test_get_person_detail_names_only_public(client, api_key, person_fixture):
    ...
    names = r.json()["names"]
    for n in names:
        assert n["id"] != person_fixture["hidden_name_id"], "hidden name leaked in detail response"
